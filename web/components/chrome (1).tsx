...
import { IS_TESTNET, chain } from "@/lib/config";
...
export function ConnectButton({ full = false }: { full?: boolean }) {
  const { address, walletName, onRightNetwork, wallets, connecting, connect, disconnect, switchNetwork } = useWallet();
    ...
    try {
      ...
      setError(
        code === 4001
          ...
          : `${w.name} could not connect to ${chain.name}. It may not support this network yet; try MetaMask, Rabby or Coinbase Wallet.`
      );
    }
  }
